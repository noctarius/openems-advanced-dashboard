...
export const useOpenEms = defineStore("openems", () => {
    ...
    const isConfigured = computed<boolean>(() =>
        restApiBase !== undefined && jsonApiAddr !== undefined
    );
    ...
    const start = async () => {
        ...
        componentCacheUpdateInterval = window.setInterval(
            () => updateChannelItemCache(),
            1000
        );
        status.value = 'ready';
    ...
    const selectComponents = (componentType: string): string[] => {
        ...
        return Object.keys(componentCache.value)
            .filter(component => component.startsWith(componentType));
    }
    ...
    const readBatteries = (): Battery[] => {
        ...
        const cells = addresses
            .filter(item => cellIdExtractor.test(item))
    ...
    const readPhotovoltaicPlanes = (): PhotovoltaicPlane[] => {
        ...
        const planes = chargers
            .map(charger => mapPhotovoltaicPlane(charger, readComponentProperty));
